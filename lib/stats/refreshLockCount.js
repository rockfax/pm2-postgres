...
module.exports = function refreshLockCount(metrics, pgClient) {
  ...
  pgClient.query(queryString, function (err, rows) {
    ...
    var accessShareLockCount = _.get(_.findWhere(rows, {mode: 'AccessShareLock'}), 'count', 'N/A');
    metrics.accessShareLockCount.set(accessShareLockCount);

    // # of Exclusive Locks
    var exclusiveLockCount = _.get(_.findWhere(rows, {mode: 'ExclusiveLock'}), 'count', 'N/A');
